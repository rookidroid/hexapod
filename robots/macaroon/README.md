...
5. **Open and Configure Code**:
   ...
   - Select this robot in [`robot.h`](../../software/hexapod_esp32/robot.h): `#define ROBOT_MACAROON` must be the only uncommented `ROBOT_*` line. The same firmware drives every robot in the family; this picks Macaroon's motion tables and settings.
   ...
   - WiFi (default: SSID="hexapod_macaroon", password="hexapod_1234"): the SSID is `APSSID` in [`src/robots/macaroon/robot_config.h`](../../software/hexapod_esp32/src/robots/macaroon/robot_config.h), the password is `APPSK` in [`config.h`](../../software/hexapod_esp32/config.h)
...
Macaroon is one of several hexapods that share the same firmware and tools. Its hardware lives in this folder; the software is common to all of them.

```text
hexapod/
├── robots/
│   └── macaroon/           # This robot: README, 3d print/ (STL + Bambu Studio project), images/
...
└── software/
    ...
    │   └── src/robots/macaroon/ # Macaroon's robot_config.h and generated motion.h
    └── path_tool/          # Gait generator that produces motion.h
        └── robots/macaroon.json # Macaroon's geometry and gait parameters
```
...
The gaits in `motion.h` are generated, not hand-written. Every table is derived from [`robots/macaroon.json`](../../software/path_tool/robots/macaroon.json), which holds the leg mount positions, link lengths, postures and gait parameters. To change one:

1. Install Python with `numpy`, then edit `robots/macaroon.json` (or add a generator in `path_tool.py`).
2. From [`software/path_tool`](../../software/path_tool/), run `python generate_motion.py macaroon`. It writes `software/hexapod_esp32/src/robots/macaroon/motion.h` directly. `lut_generator.ipynb` does the same interactively.
...
| ------- | ------- | ------------------ |
| WiFi SSID | `hexapod_macaroon` | `APSSID` in `src/robots/macaroon/robot_config.h` |
...
- Verify the SSID (`src/robots/macaroon/robot_config.h`) and password (`config.h`) (default: hexapod_macaroon/hexapod_1234)
