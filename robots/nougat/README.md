# <img src="./images/hexapod-logo.svg" alt="logo" width="128"/> Hexapod Nougat
...
<img src="./images/nougat_1.jpg" alt="hexapod_photo" width="600"/>
...
| **Actuators** | 18 × 21G digital servos, 180° travel |
| **Controller** | ESP32 with dual PCA9685 PWM drivers (I²C `0x40` / `0x41`); Raspberry Pi Pico W/2W firmware is also included |
| **Power** | 2 × 18650 Li-ion in series (2S1P) — 7.4 V nominal, 8.4 V fully charged |
...
- **Beginner-Intermediate**: Mechanical assembly skills required. No soldering is needed with the pre-assembled [Controller Board](https://rookidroid.com/product/hexapod-controller-board-mochi-esp32/) — the servos, switch and battery leads plug straight in.
...
All STL files are located in the [`3d print`](./3d%20print/) folder, along with a ready-to-print Bambu Studio project (`nougat.3mf`).
...
- **Required Libraries** (install via Arduino Library Manager):
  ...
  - ESP32: `AsyncUDP`, `ArduinoOTA`, `EEPROM` and `WebServer` (included with arduino-esp32)
...
5. **Open and Configure Code**:
   ...
   - Select this robot in [`robot.h`](../../software/hexapod_esp32/robot.h): `#define ROBOT_NOUGAT` must be the only uncommented `ROBOT_*` line. The same firmware drives every robot in the family; this picks Nougat's motion tables and settings.
   ...
   - WiFi (default: SSID="hexapod_nougat", password="hexapod_1234"): the SSID is `APSSID` in [`src/robots/nougat/robot_config.h`](../../software/hexapod_esp32/src/robots/nougat/robot_config.h), the password is `APPSK` in [`config.h`](../../software/hexapod_esp32/config.h)
...
3. **Open and Configure Code**:
   - Open [`./software/hexapod_pico/hexapod_pico.ino`](../../software/hexapod_pico/hexapod_pico.ino)
   - Edit [`config.h`](../../software/hexapod_pico/config.h) for WiFi credentials (default: SSID="hexapod", password="hexapod_1234")
...
Nougat is one of several hexapods that share the same firmware and tools. Its hardware lives in this folder; the software is common to all of them.

```text
hexapod/
├── robots/
│   └── nougat/             # This robot: README, 3d print/ (STL + Bambu Studio project), images/
...
└── software/
    ...
    │   └── src/robots/nougat/ # Nougat's robot_config.h and generated motion.h
    ├── hexapod_pico/       # Legacy Raspberry Pi Pico firmware
    └── path_tool/          # Gait generator that produces motion.h
        └── robots/nougat.json # Nougat's geometry and gait parameters
```
...
The gaits in `motion.h` are generated, not hand-written. Every table is derived from [`robots/nougat.json`](../../software/path_tool/robots/nougat.json), which holds the leg mount positions, link lengths, postures and gait parameters. To change one:

1. Install Python with `numpy`, then edit `robots/nougat.json` (or add a generator in `path_tool.py`).
2. From [`software/path_tool`](../../software/path_tool/), run `python generate_motion.py nougat`. It writes `software/hexapod_esp32/src/robots/nougat/motion.h` directly. `lut_generator.ipynb` does the same interactively.
...
| ------- | ------- | ------------------ |
| WiFi SSID | `hexapod_nougat` (ESP32), `hexapod` (Pico) | `APSSID` in `src/robots/nougat/robot_config.h` |
...
- Verify the SSID (`src/robots/nougat/robot_config.h`) and password (`config.h`) (default: hexapod_nougat/hexapod_1234 for ESP32, hexapod/hexapod_1234 for Pico)
...
The ESP32 firmware includes a **web-based calibration interface** that makes the calibration process much easier - no need to repeatedly edit code and re-upload firmware!

_For the Pico version, adjust `left_offset_ticks` and `right_offset_ticks` in [`config.h`](../../software/hexapod_pico/config.h) and re-upload the firmware instead._
...
3. **Connect to WiFi**: Join the hexapod's WiFi network (default SSID: `hexapod_nougat`)
